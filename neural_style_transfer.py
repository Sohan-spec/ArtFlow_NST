...
def build_loss(neural_net,optimizing_img,target_representations,content_feature_maps_index,style_feature_maps_indices,config):
    ...
    for  (gram_target, gram_current) in zip(target_style_representation, current_style_representation):
        style_loss +=torch.nn.MSELoss(reduction='sum')(gram_target, gram_current)# as sum penalizes more than mean
...
def neural_style_transfer(config):
    ...
    if config['initial_method']=='random':
        random_noise_image=np.random.normal(loc=0,scale=90.0,size=content_image.shape).astype(np.float32)
    ...
    if config['optimizer']=='lbfgs':
        ...
        def closure():
            ...
            if total_loss.requires_grad:
                total_loss.backward()
            with torch.no_grad():
                print(f"using LBFGS, iteration : {cnt}, total_loss : {total_loss.item():.4f}, content_loss : {config['content_weight'] * content_loss.item():.4f}, total_variaton loss : {config['tv_weight'] * tv_loss.item():.4f}")
    ...
    if config['saving_freq'] > 0:
        print("Generating video from saved frames...")
        create_video_from_results(dump_path, config['img_format'])
    else:
        print("Skipping video generation (saving_freq = -1)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--style_weight",type=float,help="Weights for style loss",default=3e4)
    ...
    parser.add_argument("--saving_freq",type=int,help="how often your images get saved (cycles), choose -1 for only saving the last output image",default=-1)
